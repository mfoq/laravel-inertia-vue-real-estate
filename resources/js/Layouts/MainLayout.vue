    ...
    <div class="container mx-auto">
      <nav class="p-4 flex items-center justify-between">
        ...
        <div class="text-xl text-indigo-600 dark:text-indigo-300 font-bold text-center">
          <Link :href="route('listing.index')">MahmoodSouq</Link>
        </div>
        <div v-if="user" class="flex items-center gap-2">
          <Link class="text-gray-500 relative" :href="route('notification.index')">
            🔔
            <div v-if="notificationCount" class="absolute left-2 bottom-3 w-4.5 h-4.5 bg-red-700 dark:bg-red-400 text-white font-medium border border-white dark:border-gray-900 rounded-full text-xs text-center">
              {{ notificationCount }}
            </div>
          </Link>
...
// page.props.value.flash.success => this is how to reach shared data in all vue pages by middleware
const page = usePage()
